...
then
    echo '  Bug board:'
    curl -s --request POST ${JIRA_API_URL}/search -u "${JIRA_API_USER}:${JIRA_API_TOKEN}" --json '{"jql":"status NOT IN (Done, Closed) AND project IN (BT) AND assignee IN (currentUser(), empty) AND type != \"New Feature\" AND status != \"Blocked / On Hold\" AND \"Team[Dropdown]\" = \"Payments development\" ORDER BY created DESC"}' |  jq -r '.issues | map({ key: .key, url: "https:\/\/talentdesk.atlassian.net\/browse\/\(.key)", priority: .fields.priority.name } | "\t\(.url) \(.priority)") []'
fi
