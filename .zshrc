...
# ZSH_THEME_RANDOM_CANDIDATES=( "simple" "fletcherm" "gozilla" "garyblessington" )
...
plugins=(git docker docker-compose fzf ubuntu debian ufw colorize command-not-found aws jsontools node nvm npm redis-cli sudo systemd themes)
...
pull_request () {
    hub pull-request -b master -h "$1" -m "$2 [master]"
    hub pull-request -b next -h "$1" -m "$2 [next]"
    hub pull-request -b staging2 -h "$1" -m "$2 [staging2]"
}
...
export PATH=$PATH:$HOME/.apparatus/scripts/ffmpeg
